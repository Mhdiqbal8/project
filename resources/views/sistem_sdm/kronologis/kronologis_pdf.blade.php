...
<body>
    <div class="header">
        <img src="{{ public_path('storage/rskk-logo2.png') }}" alt="Logo RSKK">
        <div class="title">
            <h2>FORM KRONOLOGIS PASIEN</h2>
            <p>Rumah Sakit Keluarga Kita</p>
            <p>Dicetak: {{ Carbon::now()->format('d-m-Y') }}</p>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <table class="table">
            <tr>
                ...
                <td>: {{ $fmtDate($form->tanggal ?? null) }}</td>
            ...
            <tr>
                ...
                <td>: {{ isset($form->usia) && $form->usia !== '' ? $form->usia.' tahun' : '-' }}</td>
            </tr>
        </table>
    </div>
    ...
    <div class="section">
        ...
        <div>
            {!! !empty(trim(strip_tags($form->deskripsi ?? ''))) ? ($form->deskripsi) : ($form->masalah ?? '-') !!}
        </div>
    </div>
    ...
    <div class="ttd-block">
        ...
        @if($ttdFull && file_exists($ttdFull))
            <img src="{{ $ttdFull }}" alt="TTD Petugas">
        @else
