...
<div class="container mt-4">
  ...
  {{-- ====== FLASH ====== --}}
  @if(session('success'))
    <div class="alert alert-success rounded-xl shadow-sm">{{ session('success') }}</div>
  @endif
  @if(session('error'))
    <div class="alert alert-danger rounded-xl shadow-sm">{{ session('error') }}</div>
  @endif
  ...
  <div class="d-flex justify-content-between align-items-start mb-3">
    ...
    <div class="action-bar">
      <a href="{{ route('bap.index') }}" class="btn btn-outline-secondary btn-sm rounded-xl">
      ...
      @endphp
     @unless($isMutu)
  <a href="{{ route('bap.cetak', $form->id) }}"
...
@endunless
  ...
  @if ($isMutu)
    @if ($form->manager_approved_at)
      <div class="mb-3 d-flex gap-xs">
        ...
        @if ($form->taggedUnits->count())
          <form action="{{ route('bap.accMutu', $form->id) }}" method="POST" class="d-inline">
            @csrf
    ...
    @endif
  @else
    @include('sistem_sdm.bap.partials.approval-buttons', ['form' => $form, 'user' => $authUser, 'units' => $units])
  @endif
  ...
  <div class="card card-soft p-4 mb-4">
    ...
    {{-- Status badges (pindah ke sini, soft look) --}}
    ...
    <div class="row">
      <div class="col-md-6 mb-2">
        ...
        <div class="h6 mb-0">{{ optional($form->creator)->nama ?? '-' }}</div>
      </div>
      <div class="col-md-6 mb-2">
        <div class="soft-text">Jabatan</div>
        <div class="h6 mb-0">{{ optional(optional($form->creator)->jabatan)->nama ?? '-' }}</div>
      </div>
      <div class="col-md-6 mb-2">
        <div class="soft-text">Tanggal Dibuat</div>
        <div class="h6 mb-0">{{ $form->created_at->format('d-m-Y H:i') }}</div>
      </div>
      <div class="col-md-6 mb-2">
        <div class="soft-text">Divisi Tujuan</div>
        <div class="h6 mb-0">{{ $form->divisi_verifikasi }}</div>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-md-4"><div class="soft-text">Tindakan Medis</div><div>{{ $form->tindakan_medis ?? '-' }}</div></div>
      <div class="col-md-4"><div class="soft-text">Lain-lain</div><div>{{ $form->lain_lain ?? '-' }}</div></div>
      <div class="col-md-4"><div class="soft-text">Permasalahan Lain</div><div>{{ $form->permasalahan_lain ?? '-' }}</div></div>
    </div>
    ...
    @if($form->kepala_unit_approved_at)
      <p class="mb-1">
        ...
        <strong>{{ optional($form->kepalaUnitUser)->nama }}</strong>
        <span class="soft-text">({{ optional(optional($form->kepalaUnitUser)->jabatan)->nama }})</span>
      </p>
    @elseif($form->supervision_approved_at)
      <p class="mb-1">
        <span class="soft-text">Supervision:</span>
        <strong>{{ optional($form->supervisionUser)->nama }}</strong>
        <span class="soft-text">({{ optional(optional($form->supervisionUser)->jabatan)->nama }})</span>
      </p>
    @endif
    <p class="mb-1">
      <span class="soft-text">Manager:</span>
      <strong>{{ optional($form->managerUser)->nama ?? '-' }}</strong>
      <span class="soft-text">({{ optional(optional($form->managerUser)->jabatan)->nama ?? 'Manager' }})</span>
    </p>
    @php
    ...
    <!-- <p class="mb-0">
      ...
      @if($finalReady)
        <strong>{{ optional($form->finalUser)->nama }}</strong>
        <span class="soft-text">({{ optional(optional($form->finalUser)->jabatan)->nama ?? '-' }})</span>
      @else
  ...
  <div class="card card-soft p-4 mb-5">
    <div class="d-flex align-items-center justify-content-between mb-2">
      ...
      @can('write-kronologis', $form)
        @if (!$form->final_approved_at)
          <a href="{{ route('kronologis.create', $form->id) }}"
    ...
    @if ($visibleKron->count())
      <ul class="list-group list-group-flush">
        @foreach ($visibleKron as $kron)
          <li class="list-group-item kron-item">
            <div>
              <a href="{{ route('kronologis.view', $kron->id) }}" class="text-dark text-decoration-none">
                @if($kron->tipe_kronologis === 'Medis')
                  🧑 <strong>{{ $kron->nama_pasien ?? '-' }}</strong> | RM: {{ $kron->no_rm ?? '-' }} |
                @else
                  🧾 <strong>{{ $kron->judul }}</strong> |
                @endif
                Tgl: {{ optional($kron->created_at)->format('d-m-Y') }}
              </a>
              <div class="meta">✍️ Dibuat oleh: <strong>{{ $kron->creator->nama ?? '-' }}</strong></div>
            </div>
            ...
            <div class="d-flex align-items-center gap-xs">
              ...
              @if($canSeeMutuRead)
                <span class="badge {{ $kron->mutu_checked_at ? 'badge-soft-info' : 'badge-soft-secondary' }}">
                  {{ $kron->mutu_checked_at ? 'DIBACA MUTU' : 'BELUM DIBACA MUTU' }}
                </span>
              @endif
              ...
              @if (
                ...
                <a href="{{ route('kronologis.edit', $kron->id) }}"
...
<div class="modal fade" id="tagUnitModal" tabindex="-1" aria-labelledby="tagUnitModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <form action="{{ route('bap.tagUnit', $form->id) }}" method="POST">
      @csrf
        ...
        <div class="modal-body">
          ...
          <div class="unit-box">
            <div class="row g-2 row-cols-1 row-cols-sm-2 row-cols-md-3">
              @forelse ($unitsSorted as $unit)
                <div class="col mb-2">
                  <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="unit_ids[]" value="{{ $unit->id }}" id="unit{{ $unit->id }}"
                           {{ $form->taggedUnits->contains('id', $unit->id) ? 'checked' : '' }}>
                    <label class="form-check-label" for="unit{{ $unit->id }}">{{ $unit->nama_unit }}</label>
                  </div>
                </div>
              @empty
...
(function(){
  ...
  async function loadLogs(force=false){
    ...
    try {
      const url = `{{ route('bap.tagLogs', $form->id) }}`;
