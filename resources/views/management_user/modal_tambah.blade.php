...
<div class="modal fade" id="tambah" tabindex="-1" role="dialog" aria-labelledby="modalTambahUserLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content rounded-4 shadow">
      ...
      <form action="{{ url('management_user/store') }}" method="POST" enctype="multipart/form-data">
        @csrf
              ...
              @foreach ($genders as $gender)
                <div class="form-check form-check-inline">
                  <input class="form-check-input" type="radio"
                         name="gender_id" id="gender_{{ $gender->id }}"
                         value="{{ $gender->id }}" required>

                  <!-- data-initial = L / P (atau huruf pertama nama gender) -->
                  <label class="form-check-label gender-pill"
                         for="gender_{{ $gender->id }}"
                         data-initial="{{ strtoupper(mb_substr($gender->nama, 0, 1)) }}"
                         title="{{ $gender->nama }}">
                    <span class="sr-only">{{ $gender->nama }}</span>
                  </label>
                </div>
              @endforeach
            ...
            <div class="col-md-6">
              ...
              <select name="kepala_unit_id" class="form-control">
                ...
                @foreach($allUsers as $u)
                  <option value="{{ $u->id }}">{{ $u->nama }} - {{ $u->unit->nama_unit ?? '' }}</option>
                @endforeach
              </select>
