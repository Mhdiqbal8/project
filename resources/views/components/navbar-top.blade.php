  ...
  <div class="d-flex justify-content-between align-items-center w-100 pe-5">
    ...
    <ul class="navbar-nav align-items-center ml-auto">
      ...
      <li class="nav-item dropdown">
        <a class="nav-link position-relative" href="#" id="notifDropdown" role="button"
          ...
          <span
            ...
            style="position:absolute; top:-4px; right:-6px; {{ $notifCount == 0 ? 'display:none;' : '' }}">
            {{ $notifCount }}
          </span>
        </a>
        ...
        <ul class="dropdown-menu dropdown-menu-right shadow p-2"
          ...
          @if($notifCount > 0)
            <li class="px-2 pb-1">
              <form action="{{ route('notif.read_all') }}" method="post">
                @csrf
          ...
          @forelse($unreadNotifications as $notif)
            ...
            <li class="mb-1">
              <a class="dropdown-item small border rounded p-2" href="{{ route('notif.go', $notif->id) }}">
                <div class="d-flex flex-column">
                  <strong class="text-dark">
                    <span class="mr-1">{{ $emoji }}</span>{{ $notif->data['title'] ?? '📌 Notifikasi' }}
                  </strong>
      ...
      <li class="nav-item dropdown position-relative">
        ...
        {{-- ⚠️ Modal dipindah ke @push('modals') di bawah – biar aman dari stacking --}}
      </li>
    </ul>
  </div>
</nav>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="ModalUbahPasswordTitle">Ubah Password, {{ $user->nama }}</h5>
